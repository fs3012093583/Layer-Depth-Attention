...
3. **行分支分数准备**: 用 $x_{norm}$ 作为 Key 和 Value（在此步骤不经额外映射）：
   $K_{row} = V_{row} = x_{norm}$ （同样拆分为多头）**（是否需要使用V=X）**
...
> [!NOTE]
> **📝 预混合归一化机制与构成探讨 (TODO)**
> - [ ] **跨层归一化有必要吗？为复制恒等连接，建议所入进入V的来源都不归一化，？？？** 
>       *(解答：有必要，横向对 $x_{current}$ 或纵向对 $V_{stack}$ 归一化可防止数值爆炸，确立点积公平性。)*，归一化只能放在分路上不能放在捷径上？？？。
...
> - [ ] **每一个token的每一层都要计算Q和V，这是相当大的计算量，这里应该退化，$O（BSD^2L^2）$，方法一，双Q策略，注意力矩阵融合注意细节**
...
2. **RMSNorm**: 沿着维度 $D$ 做归一化。
   $V_{norm} = \text{RMSNorm}(V_{stack})$这里应该可以考虑取消，不建议归一化
...
> - [ ] **而且列也需要先归一化，既然这样我们可以把左右的归一化放在模块内部的最后，这样可以避免重复归一化。建立完全通畅的网络，从从x到y中的任意两个节点自己接相连 **
> - [ ] **两步可以一起算，纯算法步骤上应该可以优化。**
> - [ ] **把归一化接在后面。**
> - [ ] **缺少横竖向的位置识别，也就是说q不知道所有的k的相对位置，尤其是竖向，换用潜在注意力可能解决这个问题，设计二维位置编码，可能位置编码应该去掉或者优化，影响优化速度
> 
...
4. **计算列深度注意力分数 (Memory Scores)**：
   提取自第一层堆叠到现在层之前的历史状态 $\mathcal{H} \in \mathbb{R}^{B \times S \times L_{past} \times D}$。
...
- 更细地说，这个问题主要发生在**纵向 depth 分支**，而不是横向 row 分支：
  - 横向 row 分支中的 `x_current` 已经带有 token 位置嵌入，且还有 causal mask，因此并不是完全缺少位置信息；
  - 真正缺的是**竖向历史项的显式深度身份**，也就是当前查询并不知道某个历史槽位来自浅层还是深层。
- 因此第一优先级不建议直接改成“潜在注意力”或引入更重的 latent 结构，因为 latent 只能改变压缩/聚合方式，并不会自动提供深度身份信息。
- 更合理的第一步对照应是：
  1. 给 depth 候选加入简单的 layer/depth embedding；
  2. 或在深度分数上加入 depth relative bias；
  3. 再和当前无显式深度标识的版本做收敛速度与最终指标对比。
- 后续如果要做对照，可考虑：
  1. 维持当前无显式深度标记的版本
